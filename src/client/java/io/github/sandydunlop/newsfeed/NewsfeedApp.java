...
public class NewsfeedApp extends CupraApp implements RssUpdateListener, CListBoxSelectionChangedListener {
    ...
    public void main(String[] args) {
		...
		articleScreen = new CContainer(null);
		footer = new CContainer(null, true);

		inbox = new CListBox(articleScreen,  selectionChanged -> {
		...
		openButton = new CButton(footer, "newsfeed.article.open.button", (btn) -> {
			Util.getOperatingSystem().open(article.link);
		});

		optionsButton = new CButton(footer, "newsfeed.article.options.button", (btn) -> {
			Screen screen = NewsfeedClientModInitializer.getConfigScreen(this);
			MinecraftClient.getInstance().setScreen(screen);
		});

		closeButton = new CButton(footer, "newsfeed.article.close.button", (btn) -> {
			this.close();
		});
